...
void prosessiToteutus(int maara) {
    ...
    for (int i = 0; i < maara; i++) {
        pid_t pid = fork();
        if (pid == 0) {
            cout << "Prosessi " << getpid() << " aloittaa!\n";
            aloitaRotta();
            cout << "Prosessi " << getpid() << " valmis!\n";
            exit(0);
        }
        else if (pid > 0) {
            prosessit[i] = pid;
...
int main() {
    ...
    while (tila != "p" && tila != "s") {
        cout << "Virheellinen syöte! Anna 'p' prosesseille tai 's' säikeille: ";
        cin >> tila;
    }

    cout << "Anna rottien määrä (positiivinen kokonaisluku): ";
    cin >> maara;
    while (cin.fail() || maara <= 0) {
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cout << "Virheellinen määrä! Anna positiivinen kokonaisluku: ";
        cin >> maara;
    }
    ...
    if (labyrintti == (void*) -1) { perror("shmat"); return 1; }

    memcpy(labyrintti, alkupLabyrintti, jaettuMuisti);

    if (tila == "s") {
        cout << "\nAjetaan säikeillä (" << maara << " rottaa)\n";
        vector<pthread_t> säikeet(maara);

        for (int i = 0; i < maara; i++) {
            pthread_create(&säikeet[i], nullptr, säieToteutus, nullptr);
        }
        for (int i = 0; i < maara; i++) {
        ...
        cout << "\nKaikki säikeet valmiina!\n";
    } 
    else if (tila == "p") {
        cout << "\nAjetaan prosesseilla (" << maara << " rottaa)\n";
        prosessiToteutus(maara);
        shmdt(labyrintti);
        shmctl(shm_id, IPC_RMID, nullptr);
    }

    cout << "Kaikki rotat ulkona!\n";
    return 0;
}
    //aloitaRotta();
